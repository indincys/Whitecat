...
- Developer ID 正式签名构建可直接在 app 内检查、下载并安装更新，不依赖浏览器重新下载安装
...
发布时建议先上传 GitHub Release 资产，再对外发布 GitHub Pages 上的 `appcast.xml`，避免 app 提前读到尚未可下载的安装包。更多细节见 `docs/release.md`。

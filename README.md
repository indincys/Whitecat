...
- 官方 release 由本地 `./Scripts/release.sh` 一键生成；正式签名构建可直接在 app 内检查、下载并安装更新，不依赖浏览器重新下载安装
...
Whitecat 现在采用和 Learningcat 一样的“本地发布机脚本”模型。正式发版直接在你的 Mac 上执行：
...
- 运行本地打包和签名
- 生成带 Sparkle 签名的 `appcast.xml`
- 更新 `docs/appcast.xml`
- 提交 appcast 变更
- 创建或复用 `v0.1.8` tag
- 上传 ZIP / DMG / appcast 到 GitHub Release
- 最后推送 `main`，让 GitHub Pages 发布新的 appcast

这样做不再依赖 GitHub Actions secrets。只要当前机器具备 Sparkle 私钥和签名身份，之后用户安装官方 release，就可以直接在 app 内检查、下载并安装后续更新。

当前仓库默认会优先读取本机 `~/.config/whitecat/sparkle_private_key` 作为 Sparkle 私钥，无需每次手动传 `PRIVATE_ED_KEY_PATH`。

注意：2026-03-11 起 Whitecat 使用了一套新的 Sparkle 更新密钥。2026-03-11 之前安装的旧版本如果还内置旧公钥，需要先手动安装一次新版本，之后才能继续走应用内更新。
...
自动发布流程已经按这个顺序处理：先上传 GitHub Release 资产，再推送 `docs/appcast.xml`。更多细节见 `docs/release.md`。

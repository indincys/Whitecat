...
3. 把 Sparkle 私钥保存在本机，例如：`$HOME/.config/whitecat/sparkle_private_key`。`Scripts/release.sh` 会默认优先读取这个路径。
...
- 如果本机存在 `~/.config/whitecat/sparkle_private_key`，可以不显式传 `PRIVATE_ED_KEY_PATH`。
...
- Whitecat 在 2026 年 3 月 11 日切换到了新的 Sparkle 更新密钥。
- 2026 年 3 月 11 日之前安装、且内置旧 `SUPublicEDKey` 的版本，无法直接信任新密钥签名的更新包。
- 这类旧版本需要先手动下载并安装一次新版本；完成这次过渡后，后续版本就可以继续使用应用内检查、下载和安装更新。
